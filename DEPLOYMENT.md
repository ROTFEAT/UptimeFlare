...
3. 在 **Actions** 页面允许 fork 仓库运行工作流（如 GitHub 提示），选择 **Deploy to Cloudflare → Run workflow**。
4. 确认工作流成功。以后提交到 `main` 会自动构建并更新线上部署。
...
通过 `workerConfig.notification.webhook` 设置故障和恢复通知。
支持多个 Webhook，详见 <https://github.com/lyc8503/UptimeFlare/wiki/Setup-notification>。
当前仓库是公开的：接入告警前，应使用 Secrets 和构建时注入或 Worker 服务端密钥读取方式，不能直接提交通知凭据。
